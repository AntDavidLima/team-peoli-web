      ...
      {payload.map((entry: any, index: number) => (
        <li key={`item-${index}`} className="flex items-center gap-2">
          ...
          <span className="text-lg font-bold">
            {entry.value} Treinos <br/><div className="text-sm font-light text-foreground">
...
export function PeriodSummary({ exercises }: PeriodSummaryProps) {
  const summaryData = useMemo(() => {
    ...
    const validExercisesForSummary = exercises.filter(
      (ex) => ex.workouts && ex.workouts.length >= 2
    );

    validExercisesForSummary.forEach((exercise) => {
      const workoutsByWeekForExercise = exercise.workouts.reduce((acc, workout) => {
        const weekKey = getWeekKey(new Date(workout.workout.startTime));
        if (!acc[weekKey]) acc[weekKey] = [];
        acc[weekKey].push(workout);
        return acc;
      }, {} as Record<string, typeof exercise.workouts>);

      const weekKeysForExercise = Object.keys(workoutsByWeekForExercise).sort();
      if (weekKeysForExercise.length < 2) return;

      const firstWeekKey = weekKeysForExercise[0];
      const lastWeekKey = weekKeysForExercise[weekKeysForExercise.length - 1];

      const calculateVolumeForWeek = (workouts: typeof exercise.workouts) => {
      ...
      const firstWeekVolume = calculateVolumeForWeek(workoutsByWeekForExercise[firstWeekKey]);
      const lastWeekVolume = calculateVolumeForWeek(workoutsByWeekForExercise[lastWeekKey]);

      if (firstWeekVolume === 0) {
        if (lastWeekVolume > 0) exerciseClassification.progression += 1;
        return;
      }
      
      const evolution = ((lastWeekVolume - firstWeekVolume) / firstWeekVolume) * 100;

      if (evolution >= -3 && evolution <= 0) {
        exerciseClassification.constant += 1;
      } else if (evolution > 0) {
        exerciseClassification.progression += 1;
      } else {
        exerciseClassification.regression += 1;
      }
    });

    const totalClassifiedWorkouts =
      exerciseClassification.progression +
      exerciseClassification.constant +
      exerciseClassification.regression;
    ...
    const pieChartData = [
      { name: "progression", value: exerciseClassification.progression },
      { name: "constant", value: exerciseClassification.constant },
      { name: "regression", value: exerciseClassification.regression },
    ].filter(item => item.value > 0);

    return {
      totalWorkouts: totalClassifiedWorkouts,
      pieChartData,
      ...
      <div className="flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="w-full md:w-2/3 grid grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
          <div className="space-y-1">
            ...
            <p className={`text-sm font-semibold ${summaryData.volumeChange > 0 ? 'text-green-500' : summaryData.volumeChange < 0 && 'text-red-500'}`}>
          ...
          <div className="space-y-1">
            ...
            <p className={`text-sm font-semibold ${summaryData.avgLoadChange > 0 ? 'text-green-500' : summaryData.avgLoadChange < 0 && 'text-red-500'}`}>
          ...
          <div className="space-y-1">
            ...
            <p className={`text-sm font-semibold ${summaryData.avgRepsChange > 0 ? 'text-green-500'  : summaryData.avgRepsChange < 0 && 'text-red-500'}`}>
